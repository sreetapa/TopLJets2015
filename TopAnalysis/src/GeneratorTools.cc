...
float cosThetaStar (MiniEvent_t &ev, vector<Particle> genleptons, vector<Particle> genwbosons, vector<Particle> genbs) {
  ...
  */
  for(Int_t igen=0; igen<ev.ngtop; igen++) {
	...
	//       cout << "onegenw " ;
	  ...
	  //float wgt = pdf/pdfSM;
	  wgts.push_back(costhetastar);
        }
      }
    }
  }
  if ( wgts.size() == 2) weight = wgts[0]*wgts[1];
  cout << "weight actually costhetastar  " << weight << endl; 
  cout << "costhetastar_size " << wgts.size() << endl; 
  return weight;
